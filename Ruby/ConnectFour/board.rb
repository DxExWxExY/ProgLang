...
class Board
    ...
    def pop_disc(column)
        discs = @board.transpose.slice(column).take(5)
        i = 0
        while i < 6
            @board[i][column] = nil
            i+=1
        end
        discs.reverse.each do |e|
            add_disc(e, column)
        end
    end
    ...
    def print
        puts @board.map {|row| row.map {|e| e == nil ? "( )" : "(#{e})"}.join(' ')}.join("\n")
        puts ' 1 | 2 | 3 | 4 | 5 | 6 | 7 '
    end
    ...
    def is_full?(column)
        col = @board.transpose.slice(column).rindex(nil)
        col.nil?
    end
